...
exports.all = function(req, res) {
    ...
    Category.find({ parentCategory: 0, active: true }).select({ name: 1, category: 1, parentCategory: 1, info: 1, image: 1, active: 1 }).exec(function(err, parents) {
        ...
        async.each(parents, function(a, callback) {
                ...
                Category.find({ parentCategory: parseInt(a.category), active: true }).select({ name: 1, category: 1, parentCategory: 1, info: 1, image: 1, active: 1 }).exec(function(err, c) {
                    async.each(c, function(d, callback) {
                            d = d.toObject(); //c = level1
                            Category.find({ parentCategory: parseInt(d.category), active: true }).select({ name: 1, category: 1, parentCategory: 1, info: 1, image: 1, active: 1 }).exec(function(err, e) {
...
//code foe category sub category and leaf category
...
async.waterfall([
    function(callback) {
        async.each(parents, function(a, callback){
            a = a.toObject();
            Category.find({parentCategory:parseInt(a.category), active:true}).select({name:1,category:1,parentCategory:1,image:1, active:1}).exec(function(err,c){
              a.sub_categories = c;
              p.push(a);
              callback();
            });
